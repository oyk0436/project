...
<body>
	...
	<div style = "font-size:25px;font-weight:bold;text-align:center;">
		<font color = "blue">${vo.title }</font> 이용 후기 수정
	</div>
	...
	<div style="display:table;position:relative;width:100%;float:left;top:10px;">
		...
		<div style=";display:table-row;text-align:center;">
 		...
 		<div style=";display:table-row;text-align:center;width:500px;">
			...
			<div class="cellLineReview2">
				<textarea name = "content" id = "content" style="width:98%;height:50px;" autofocus>${vo.content }</textarea>
			</div>
		</div>
		<div style=";display:table-row;text-align:center;">
			...
			<div class="cellLineReview2">
				<select name = "grade" id = "grade" style = "width:100px;">
  					<option value="5" <c:if test="${vo.grade=='5'}">selected</c:if>>5 (아주좋음)</option>
  					<option value="4" <c:if test="${vo.grade=='4'}">selected</c:if>>4 (좋음)</option>
  					<option value="3" <c:if test="${vo.grade=='3'}">selected</c:if>>3 (보통)</option>
  					<option value="2" <c:if test="${vo.grade=='2'}">selected</c:if>>2 (나쁨)</option>
  					<option value="1" <c:if test="${vo.grade=='1'}">selected</c:if>>1 (아주나쁨)</option>
  				</select>
			</div>
		</div>
 	</div>
 	...
 	<div style="position:absolute;width:100%;text-align:center;top:20px;margin-top:240px;">
		<button type = "button" name = "btn_save" id = "btn_save">수정</button>
