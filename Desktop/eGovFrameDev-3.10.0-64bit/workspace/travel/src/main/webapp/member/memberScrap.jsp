...
.div2 {
	margin:0 auto;width:700px;height:1000px;text-align:center;padding-top:0px;float:left;background-color:rgb(255, 255, 255);
}
.div_button {
...
<div class="container" style="margin:0 auto;">
	...
	<div style = "text-align:center;margin-top:20px;margin-left:100px;font-size:12px;">
		...
		<c:forEach var="i" begin="${hotel.page_sno1 }" end="${page_eno1 }">
			<a href = "memberScrap.do?page_no1=${i }"><span style="font-size:14px;">${i }</span></span></a>
		</c:forEach>
		<%
		...
		<div style="display:table;position:relative;width:700px;margin-left:50px;top:10px;border-top:2px solid #000000;">
			...
			<div style="display:table-row;text-align:center;">
				...
				<div class="membercell2" style="width:5%;">
					<a href = "javascript:void(0)" onclick="fn_rScrapDelete('${result2.unq}','${result2.title }')"><span style="font-size:12px;">삭제</span></a>
				</div>
			</div>
	...
	<div style = "text-align:center;margin-top:50px;margin-left:100px;font-size:12px;">
		...
		<c:forEach var="i" begin="${scrap2.page_sno2 }" end="${page_eno2 }">
			<a href = "memberScrap.do?page_no2=${i }"><span style="font-size:14px;">${i }</span></a>
		</c:forEach>
		<%
	...
	<span style="font-size:15px;font-weight:bold;margin-left:50px;">관광지 관심 목록</span>
		...
		<div style="display:table;position:relative;width:700px;margin-left:50px;top:10px;border-top:2px solid #000000;">
	 		<div style="display:table-row;text-align:center;">
				<div class="membercell1" style="width:5%;">번호</div>
				<div class="membercell1" style="width:25%;">관광지명</div>
				<div class="membercell1" style="width:50%;">주소</div>
				<div class="membercell1" style="width:15%;">평점</div>
				<div class="membercell1" style="width:5%;">삭제</div>
			</div>
			
			<c:set var="rownum3" value="${scrap3.rownum3 }"/>
			<c:forEach var = "result3" items = "${list3 }">
		
			<div style="display:table-row;text-align:center;">
				<div class="membercell2" style="width:5%;">${rownum3 }</div>
				<div class="membercell2" style="width:25%;">${result3.title }</div>
				<div class="membercell2" style="width:50%;">${result3.address }</div>
				<div class="membercell2" style="width:15%;">${result3.grade }점</div>
				<div class="membercell2" style="width:5%;">
					<a href = "javascript:void(0)" onclick="fn_rScrapDelete('${result3.unq}','${result3.title }')"><span style="font-size:12px;">삭제</span></a>
				</div>
			</div>
	...
	<div style = "text-align:center;margin-top:80px;margin-left:100px;font-size:12px;">
		<c:set var="page_sno3" value="${scrap3.page_sno3 }"/>
		<%
		...
		if (page_sno3-1 > 0) {
		%>
			<a href = "memberScrap.do?page_no3=${page_sno3-1 }">이전</a>
		<%
		}
		%>

		<c:set var="page_eno3" value="${scrap3.page_eno3 }"/>
		<c:set var="total_page3" value="${scrap3.total_page3 }"/>
		<%
		...
		pageContext.setAttribute("page_eno3", page_eno3) ;
			%>
		<c:forEach var="i" begin="${scrap2.page_sno3 }" end="${page_eno3 }">
			<a href = "memberScrap.do?page_no3=${i }"><span style="font-size:14px;">${i }</span></a>
		</c:forEach>
		<%
		if (page_eno3+1 < total_page3) {
		%>
			<a href = "memberScrap.do?page_no3=${scrap3.page_eno3+1 }">다음</a>
		<%
		}
		%>
	</div>
