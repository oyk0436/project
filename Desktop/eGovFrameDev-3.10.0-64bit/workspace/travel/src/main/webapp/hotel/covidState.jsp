...
<style>
body {
	font-size:9px;color:;font-family:'맑은 고딕';
}
div.header {
	display:table;position:relative;width:600px;height:30px;left:50px;top:30px;border:0px solid #ccc;
}
div.row1 {
...
div.body2 {
	display:table;position:relative;width:100%;height:30px;left:50px;top:40px;
}
div.cell2_1 {
...
div.body3 {
	display:table;position:relative;width:100%;height:50px;left:50px;top:40px;
}
div.footer {
	display:table;position:relative;width:100%;height:30px;left:50px;top:50px;
}
div.cell3_1 {
...
<script>
	...
	function fn_area(n) {
		location = "covidState.jsp?area="+ encodeURI(n , "UTF-8"); // get 방식 한글깨짐 방지
	}
</script>
...
<body>
	...
	<div class = "header">		
		<div class = "row1">			
			<div style = "display:table-cell;font-size:20px;font-weight:bold;padding:7px;">
				<font size = "5" color = "purple"><%=areaFullName %></font> 확진자 현황
			</div>		
	...
	<div class = "body2">			
		<div style = "display:table-row; text-align:center;">
			<div class = "cell2_1"><a href = "javascript:fn_popup()" style = "color:black;">거리두기 단계</a></div>
			<div class = "cell2_2">기준 일시</div>
			<div class = "cell2_3">비고</div>
		</div>
	</div>	
		<% 
		FileReader file = new FileReader("C:\\Temp/distance.csv"); // csv file 저장된 장소
		BufferedReader buffer = new BufferedReader(file);
		
		String data = "";
		String distanceArea = ""; // 지역
		String upDate = ""; // 기준 일시
		String distanceStep = ""; // 거리두기 단계

		while (true) {
			data = buffer.readLine();
			if (data != null) {
				distanceArea = data.split(",")[1].trim();
				upDate = data.split(",")[0].trim();
				distanceStep = data.split(",")[2].trim();
				
				if (distanceArea.equals(area)) {
				%>
				<div class = "body3">
					<div style = "display:table-row; text-align:center;">
						<div class = "cell6_1">
							<a href = "javascript:fn_popup()" style = "color:red;">
								<%=distanceStep %> <font color = "black">단계</font> 
							</a> <br>
							
							<%
							if (distanceStep.equals("1")) {
								out.print("( 지속적 억제상태 유지 )");
							} else if (distanceStep.equals("2")) {
								out.print("( 지역유행 / 인원 제한 )");
							} else if (distanceStep.equals("3")) {
								out.print("( 권역 유행 / 모임 금지 )");
							} else if (distanceStep.equals("4")) {
								out.print("( 대유행 / 외출금지 )");
							}
							%>
						</div>
						<div class = "cell6_2"><%=upDate %></div>
						<div class = "cell6_3" style = "text-align:left;padding-left:7px;">
						<%
						String[] noteStr = data.split("-");
						for (int i = 1; i < noteStr.length; i++) {
							out.print(" - " + noteStr[i] + "<br>");
						}
					}
					%>
						</div>
	...
	<div class = "footer" style = "margin-top:-10px;">			
		<div style = "display:table-row;">
			<div class = "cell3_1">* 거리두기 단계를 클릭하시면 단계별 상세보기 팝업창이 열립니다.</div>
		</div>
	</div>	
	<%
	}
